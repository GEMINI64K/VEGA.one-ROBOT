...
void loop() {
  ...
  if (currentMillis - prevMillis >= tiltInterval && tiltPos < maxTiltPos) {
  ...
  // Break out of the loop when the tilt position reaches its maximum limit
  if (tiltPos >= maxTiltPos) {
    Serial.println("Tilt has reached its maximum limit. Stopping the loop.");
    while (true) {
      // Stuck in an infinite loop to halt further execution
    }
  }
}
